...
#######################################
est.dtpn0<-
function (d=data,l0=0.5,sl=0.1,n=10,TL=0,mod=0.8,com=2,c2=1,TR2=max(d),TR1=round(max(mod+c2*(10^(-com)),mod+(min(quantile(d,0.95),TR2)-mod)*0.1),com),
I=100,I1=25,T=max(d),nen=1,epsilon1=1E-4,steptol=4) 
{
#############################
# function est.dtpn0 (parameter estimation of TPN distribution to the data truncated at different points)  written on 12.03.2009
# for two-sided 
# PN(lambda,mu,sigma)
...
while(i<=m)
         ...
         v<-KD.tnorm1(data=d3,p=frac,mu=mu,sig=sig,T1=T11,crit=crit1)
         v1<-KD.tnorm2(data=d3,p=frac,mu=mu,sig=sig,crit=crit1)
         wv<-w+v+v1
         if (wv >=1 || is.na(wv))
            {
            y[i,1:12]<-c(rep(1,12))
         ...
         {
         wvinv<-1/wv
         y[i,1:13]<-c(q1,qu2,TL,tr2,p,mu,sig,lam,ll,w,v+v1,wv,wvinv)
         }
         }
         i<-i+1;
       }
